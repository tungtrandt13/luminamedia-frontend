...
export default function HPContact({ title, description, ctaText }: Props) {
  ...
  if (success) {
    return (
      <section className="w-full bg-[#FFF8ED] text-[#171717]">
        <div className="mx-auto w-full max-w-[1240px] px-5 py-[100px] text-center">
          <h2 className="text-[40px] font-semibold text-[#AF7E2D] italic">{t('success')}</h2>
          <p className="mt-4 text-[18px] text-[#6C6C6C] font-light">{t('success_desc')}</p>
          <button
            onClick={() => setSuccess(false)}
            className="mt-8 text-[#AF7E2D] font-semibold underline underline-offset-4 hover:opacity-80 transition-opacity"
          >
            {t('submit_another')}
          </button>
        </div>
