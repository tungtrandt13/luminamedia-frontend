    ...
    return (
        <section className="w-full bg-[#000000] text-white py-[60px] md:py-[100px] overflow-hidden">
            <div className="mx-auto w-full max-w-[1240px] px-5 flex flex-col gap-10 md:gap-[80px]">
                ...
                {/* Testimonials List */}
                <div className="flex overflow-x-auto snap-x snap-mandatory lg:grid lg:grid-cols-3 gap-[22px] pb-8 lg:pb-0 scrollbar-hide">
                            ...
                            <div className="flex items-center gap-[15px] mt-auto">
                                <img
                                    ...
                                    onError={(e) => {
                                        ...
                                        target.style.display = 'none'; // Phụ thuộc vào việc có muốn ẩn luôn hay không, ta cứ để ẩn cho gọn hoặc hiện nền trắng
                                    }}
                                />
                ...
                <div className="flex justify-center mt-10 gap-2">
                    {Array.from({ length: 6 }).map((_, idx) => (
                        <div
                            key={idx}
                            className={`w-[12px] h-[12px] rounded-full transition-all duration-300 ${idx === 0 ? 'bg-[#AF7E2D]' : 'bg-[#D9D9D9]'}`}
                        />
                    ))}
                </div>
